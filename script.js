...
function findGrays() {
  ...
  for(let i = 0; i < wordle.length; i++){
    if(!wordle.includes(userAttempt.charAt(i))){
      grayIndexes.push(i);
    }
  }
...
function findYellows() {
  ...
  for(let i = 0; i < wordle.length; i++){
    if(wordle.includes(userAttempt.charAt(i)) && wordle.charAt(i) !== userAttempt.charAt(i)){
...
function findGreens() {
  ...
  for(let i = 0; i < wordle.length; i++){
    if(wordle.charAt(i) === userAttempt.charAt(i)){
      greenIndexes.push(i);
    }
  }
...
function keyPressed(event) {
  if (currentTileIndex < currentInputRow.children.length) {
    //if the current tile index smaller bigger than the indexes of the tiles
